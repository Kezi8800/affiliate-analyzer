...
function isConsumerReports(params = {}) {
  const text = Object.keys(params).map(k => `${k}=${params[k]}`).join("&").toLowerCase();
  return text.includes("consumerreports") || text.includes("consumer reports") || text.includes("consumer-reports");
}
...
function buildConsumerReportsResult(inputUrl, urlObj, params) {
  ...
  return {
    ...
    publisher_intelligence: {
      ...
      media_group: mediaGroup,
      parent_media_group: mediaGroup,
      confidence: "high",
      matched_by: "utm_campaign_consumerreports",
      matched_pattern: "utm_campaign=consumerreports",
      network,
    ...
    signals: {
      ...
      hasSubId: Boolean(getParam(params, "clickref") || getParam(params, "click_ref")),
...
function buildAmazonAttributionMaasResult(inputUrl, urlObj, params) {
  ...
  return {
    ok: true,
    error: false,
    version: "BrandShuo Analyze v4.5 Amazon Attribution MAAS",
    engine: "BrandShuo Attribution Intelligence Engine",
...
function buildFuturePublishingResult(inputUrl, urlObj, params) {
  ...
  return {
    ok: true,
    error: false,
    version: "BrandShuo Analyze v4.5 Future Publishing",
    engine: "BrandShuo Attribution Intelligence Engine",
    ...
    publisher_intelligence: {
      ...
      confidence: "high",
      matched_by: "future_publishing_signal",
      matched_pattern: "futurepublishing / future publishing / Future-owned publisher",
      network,
      network_type: "Affiliate Network",
...
function buildPartnerizePepperjamResult(inputUrl, urlObj, params) {
  ...
  return {
    ok: true,
    error: false,
    version: "BrandShuo Analyze v4.5 Partnerize",
    engine: "BrandShuo Attribution Intelligence Engine",
