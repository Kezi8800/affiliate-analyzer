 ...
 * lib/analyze.js
 * v3.0
 * Final merged version with:
 * - Amazon publisher engine
 * - Generic publisher engine (Impact / CJ / Awin / Partnerize / etc.)
 */
...
/**
 * Amazon fallback rule set
 * Used only when new Amazon engine does not hit
 */
const AMAZON_PUBLISHER_RULES = [
  ...
  { pattern: /capitaloneshopping|wikibuy/i, publisher: "Capital One Shopping", type: "Coupon / Browser Extension", media_group: "Coupons & Deals", subtype: "Browser Commerce" },
...
function detectNetwork(params = {}, hostname = "") {
  ...
  if (
    "irclickid" in params ||
    "irgwc" in params ||
    "cidimp" in Object.keys(params).reduce((acc, key) => {
      acc[toLower(key)] = true;
      return acc;
    }, {}) ||
    h.includes("impactradius") ||
    h.includes("impact.com")
  ) {
    return "Impact";
  }
...
function buildAttributionConflict(params = {}) {
  ...
  if ("irclickid" in params || "irgwc" in params) {
    layers.push("Impact");
    risk_score += 20;
  }
  ...
  if ("clickref" in params || toLower(params.utm_source).includes("partnerize")) {
    layers.push("Partnerize");
    risk_score += 20;
  }
...
function buildFinalVerdict(network = "", commercialIntent = {}, attributionConflict = {}, publisherIntel = {}) {
  ...
  if ((network === "Amazon Associates" || network === "Impact" || network === "CJ Affiliate" || network === "Awin" || network === "Partnerize") &&
      publisherIntel.publisher &&
      publisherIntel.publisher !== "Unknown") {
    likely_closer = commercialIntent.channel_role === "Closer"
      ? publisherIntel.publisher
      : likely_closer;
...
function analyzeAmazonLink(urlObj) {
  ...
  return {
    ...
    publisher_intelligence: {
      ...
      publisher_group: publisherIntel.publisher_group || "Unknown",
...
function analyzeGenericLink(urlObj) {
  ...
  const publisherIntel = detectPublisherFromGenericSignals(params, hostname);

  const trafficQuality = scoreTrafficQuality(params, network, publisherIntel);
  const commercialIntel = buildCommercialIntent(params, network, publisherIntel);
  const attributionConflict = buildAttributionConflict(params);
  const merchantIntel = detectMerchantIntelligence(urlObj, params);
  const finalVerdict = buildFinalVerdict(network, commercialIntel, attributionConflict, publisherIntel);
  ...
  return {
    ...
    publisher_intelligence: {
      ...
      media_group: publisherIntel.media_group,
      subtype: publisherIntel.subtype,
      source_ownership: publisherIntel.publisher !== "Unknown"
        ? "Derived generic publisher signal"
        : "Unknown",
      confidence: publisherIntel.confidence,
      matched_by: publisherIntel.matched_by,
      evidence: publisherIntel.evidence || null
    },
    ...
    commercial_intelligence: {
      ...commercialIntel,
      traffic_type: publisherIntel.traffic_type && publisherIntel.traffic_type !== "Unknown"
        ? publisherIntel.traffic_type
        : commercialIntel.traffic_type,
      channel_role: publisherIntel.channel_role && publisherIntel.channel_role !== "Unknown"
        ? publisherIntel.channel_role
        : commercialIntel.channel_role,
      incrementality_risk: publisherIntel.incrementality_risk && publisherIntel.incrementality_risk !== "Medium"
        ? publisherIntel.incrementality_risk
        : commercialIntel.incrementality_risk
    },
