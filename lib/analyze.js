...
// Main Analyze Engine v4.1.2
// Fix: Saatva + Future Publishing detection + safer non-Amazon fallback
...
function buildPublisherDisplayLabel(publisher = "", mediaGroup = "") {
  ...
  if (groupName && isNetworkGroup(groupName)) return publisherName;
...
function normalizePublisherIntel(publisherResult = {}, networkSignal = null) {
  const publisher = cleanValue(publisherResult.publisher, "Affiliate Source");
...
function analyzeLink(inputUrl) {
  ...
  if (forcePartnerizePepperjam) {
    ...
    return {
      ok: true,
      version: "v4.1.2-partnerize-force",
      engine: "BrandShuo Attribution Intelligence Engine v4.1.2",
  ...
  if (
    ...
    const amazonPublisher = cleanValue(amazonResult.publisher, "Amazon Publisher");
  ...
  const utmMedium = lower(getParam(params, "utm_medium"));
  ...
  /* =========================
     ...
     Example:
     saatva.com/...utm_source=futurepublishing&utm_medium=affiliate&click_id=...
  ========================= */
  ...
  if (forceFuturePublishing) {
    ...
    publisherResult.raw = {
      matched_rule: "utm_source=futurepublishing",
      utm_source: getParam(params, "utm_source") || null,
