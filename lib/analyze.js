...
function detectAmazonAttributionSystem(params) {
  ...
  const linkCode = String(getParam(params, "linkCode") || "").toLowerCase();
  const ascsubtag = String(getParam(params, "ascsubtag") || "").toLowerCase();
  const btnRef = String(getParam(params, "btn_ref") || "").toLowerCase();
  ...
  const hasAccSignals =
    ...
    linkCode === "tr1" ||
    linkCode === "ur2" ||
    hasAnyParam(params, ["creative", "camp"]) ||
    btnRef.includes("srctok") ||
    ascsubtag.endsWith("int");

  if (hasAttributionSignals) return "Amazon Attribution";
  if (hasTag && hasAccSignals) return "Amazon Creator Connections";
...
function inferPublisherFromAmazonTag(tag) {
  ...
  if (t.includes("bf") || t.includes("buzzfeed") || t.includes("buzz")) {
    return {
      publisher: "BuzzFeed",
  ...
  if (t.includes("slickdeals") || t.includes("slick") || t.includes("sd-")) {
    return {
  ...
  if (t.includes("pcmag") || t.includes("p00935")) {
    return {
      publisher: "PCMag",
      type: "Editorial Review",
      subtype: "Commerce Review",
  ...
  if (t.includes("wirecutter") || t.includes("nytimes")) {
    return {
...
function inferPublisherFromAscSubtag(ascsubtag) {
  ...
  if (decoded.includes("pcmag") || decoded.includes("ziff")) {
    return {
...
function buildAmazonPathClassification({
  publisher,
  attribution_system,
  merchant
}) {
  ...
  let pathLabel = `${cleanPublisher} → ${merchant}`;
  let pathGroup = "Amazon Organic Path";
  let pathNodes = [cleanPublisher, merchant];

  if (attribution_system === "Amazon Creator Connections") {
    pathLabel = `${cleanPublisher} → Amazon Creator Connections → ${merchant}`;
    pathGroup = "Amazon Creator Connections Path";
    pathNodes = [cleanPublisher, "Amazon Creator Connections", merchant];
  } else if (attribution_system === "Amazon Associates") {
    pathLabel = `${cleanPublisher} → Amazon Associates → ${merchant}`;
    pathGroup = "Amazon Associates Path";
    pathNodes = [cleanPublisher, "Amazon Associates", merchant];
  } else if (attribution_system === "Amazon Attribution") {
    pathLabel = `Brand / Advertiser → Amazon Attribution → ${merchant}`;
    pathGroup = "Amazon Attribution Path";
    pathNodes = ["Brand / Advertiser", "Amazon Attribution", merchant];
  }
...
function buildAmazonAttributionLayer(urlObj) {
  ...
  return {
    engine:
      "Attribution Layer Engine v2 + Publisher Intelligence v3 Fallback + Quality & Intent Engine v2 + Amazon Program Split v2.4 + Retail Intent GMV Engine v2.3",
