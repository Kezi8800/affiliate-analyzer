...
function findPublisherByImpactId(id) {
  if (!id) return null;
  const hit = PUBLISHER_LIBRARY_V1.find(
    (item) => Array.isArray(item.impactIds) && item.impactIds.map(String).includes(String(id))
  );
  return hit || null;
}

function findPublisherByAmazonTag(tag) {
  if (!tag) return null;

  const normalized = String(tag).trim();

  const hit = PUBLISHER_LIBRARY_V1.find(
    (item) => Array.isArray(item.amazonTags) && item.amazonTags.map(String).includes(normalized)
  );
