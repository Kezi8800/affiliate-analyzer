...
function inferAmazonLinkMode(params = {}) {
  ...
  if (
    linkCode.startsWith("ur") ||
    linkCode.startsWith("ll") ||
    linkCode.startsWith("as")
  ) {
    return "Amazon Associates";
  }
...
function detectAmazonPublisherFromUrl(url = "") {
  ...
  let result = {
    ...
    tracking: {
      ...
      creative,
      creativeASIN,
      ref: params.ref_ || params.ref || null,
      th: params.th || null
    },
  ...
  // 1) tag -> 精准规则库优先
  if (tag) {
    const resolved = resolveAmazonAssociateTag(tag);
    ...
    if (resolved) {
      ...
      result.publisherName = resolved.publisherName;
      result.publisherType = resolved.publisherType;
      result.publisherSlug = resolved.publisherSlug || null;

      result.matchMethod = resolved.matchMethod;
      result.note = resolved.note || null;

      result.status = "resolved";
      result.confidence = resolved.confidence || 0.99;
      result.matchedRules.push("amazon_tag_map");
    }
  }

  // 2) 没命中 map，但有 tag，仍然先判 Amazon Associates
  if (!result.publisherName && tag) {
    result.network = "Amazon Associates";
    result.channel = "Affiliate";
    result.program = "Amazon Associates";
    result.publisherToken = tag;
  ...
  // 3) linkCode / camp / creative 作为 Amazon tracking/affiliate 信号
  ...
  // 4) 如果还没解出 publisher，用 fingerprint 做弱推断
  if (!result.publisherName) {
    ...
    if (fpResolved) {
      ...
      result.publisherSlug = fpResolved.publisherSlug || null;
      result.matchMethod = fpResolved.matchMethod;
      result.note = fpResolved.note || null;
      result.status = "resolved";
  ...
  pushEvidence(evidence, "ref", params.ref);
