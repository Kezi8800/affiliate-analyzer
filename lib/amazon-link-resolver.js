...
function inferPublisherFromAmazonTag(tag = "", ascsubtag = "") {
  ...
  const rules = [
    ...
    {
      publisher: "DealNews",
      patterns: ["dealnewscom", "dealnews"],
      type: "Deal / Coupon",
      subtype: "Deal Discovery / Editorial Commerce",
      media_group: "DealNews"
    },
    {
      publisher: "BuzzFeed",
...
function resolveAmazonLink(urlObj, rawParams = {}) {
  ...
  const attributionPartnerPublisher = attributionIds.some(value => /^lv_/i.test(value))
    ? {
        publisher: "Levanta",
        type: "Creator Commerce",
        subtype: "Creator / Affiliate Partnership",
        media_group: "Levanta",
        confidence: "High",
        matched_by: "amazon_attribution_partner_prefix",
        matched_pattern: "lv_"
      }
    : null;
  ...
  if (attribution.is_amazon_attribution) {
    ...
    primaryClaimer = publisherIntel?.publisher || "Brand / Advertiser";
  ...
  const channelRole =
    publisherIntel?.publisher === "Levanta"
