...
// Unified Publisher Detection Adapter v3.0
...
function detectAffiliateNetwork(params = {}, hostname = "") {
  ...
  if (hasAnyParam(params, ["ranmid", "ransiteid", "raneaid"]) || getParam(params, "rktevent")) {
    return "Rakuten";
  }
...
function normalizePublisherResult(result, source = "unknown") {
  ...
  return {
    ...
    group: result.group || result.media_group || "Unidentified Publisher Group",
    ...
    score: quality,
    incrementalityRisk: result.incrementalityRisk || result.risk || "Medium",
...
function detectFromCommonParams(params = {}, rawUrl = "") {
  ...
  if (includesAny(fields, ["buzzfeed", "bf-sfp", "bf-shp", "bfheather"])) {
    return normalizePublisherResult({
      matched: true,
      publisher: "BuzzFeed",
      domain: "buzzfeed.com",
  ...
  if (includesAny(fields, ["future publishing", "tomsguide", "techradar", "pcgamer", "laptopmag"])) {
    return normalizePublisherResult({
      matched: true,
  ...
  if (includesAny(fields, ["slickdeals"])) {
    return normalizePublisherResult({
      matched: true,
...
module.exports = {
  // New analyze.js / lib/analyze.js dependencies
