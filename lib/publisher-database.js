...
// Phase 2: Global publisher expansion merged

const EXPANSION = require("./publisher-database-expansion");
const EXPANSION_V2 = require("./publisher-database-expansion-v2");
const EXPANSION_V3 = require("./publisher-database-expansion-v3");
const EXPANSION_V4 = require("./publisher-database-expansion-v4");
const EXPANSION_V5 = require("./publisher-database-expansion-v5");
const EXPANSION_V6 = require("./publisher-database-expansion-v6");
const EXPANSION_V7 = require("./publisher-database-expansion-v7");
const EXPANSION_V8 = require("./publisher-database-expansion-v8");
const EXPANSION_V9 = require("./publisher-database-expansion-v9");
const EXPANSION_V10 = require("./publisher-database-expansion-v10");
...
const PUBLISHERS = [
  ...
  },
  {
    ...
    intent: "High Purchase Intent",
    intentLevel: "High Purchase Intent",
    role: "Bottom Funnel / Promo Discovery",
    funnelRole: "Bottom Funnel / Promo Discovery",
    quality: 62,
    incrementalityRisk: "Medium-High",
    attributionRisk: "Deal discovery / last-click risk",
    domains: ["dealseek.com"],
    amazonTags: ["dealseekweb-20"],
    aliases: ["dealseek", "deal seek", "dealseek web"],
...
// Merge expansions: deduplicate by id, later entries overwrite earlier ones
function mergeExpansion(expansion) {
  for (const entry of expansion) {
    if (!entry || !entry.id) continue;
    if (existingIds.has(entry.id)) {
      const idx = PUBLISHERS.findIndex(p => p.id === entry.id);
      if (idx >= 0) PUBLISHERS[idx] = entry;
    } else {
...
mergeExpansion(EXPANSION_V2);
mergeExpansion(EXPANSION_V3);
mergeExpansion(EXPANSION_V4);
mergeExpansion(EXPANSION_V5);
mergeExpansion(EXPANSION_V6);
mergeExpansion(EXPANSION_V7);
mergeExpansion(EXPANSION_V8);
mergeExpansion(EXPANSION_V9);
mergeExpansion(EXPANSION_V10);
...
function detectPublisherByUrl(rawUrl = "") {
  ...
  for (const p of PUBLISHERS) {
    ...
    const aliasHit = (p.aliases || []).some(alias => {
      ...
      if (a.length < 4 || ca.length < 4) return false;
...
function detectPublisherByNetworkSignals(input = {}) {
  ...
  const networkRules = [
    ...
    { keys: ["skimlinks", "skimresources", "redirectingat.com", "xcust"], id: "skimlinks" },
